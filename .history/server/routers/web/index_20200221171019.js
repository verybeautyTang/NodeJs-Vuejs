module.exports = app => {
    ...
    router.get('/heroes/init',async(req,res) =>{
        ...
        // 显示英雄
        router.get('/hroes/list',async (req,res) =>{
            const parent = await Category.findOne({
                name: '英雄分类'
            })
            ...
            hero.map(cat =>{
                cat.heroList.map(heros => {
                    if(cat.name==='热门'){
                        heros.categoryName = heros.categories[0].name
                    }
                    else{
                        heros.categoryName = cat.name
                    }
