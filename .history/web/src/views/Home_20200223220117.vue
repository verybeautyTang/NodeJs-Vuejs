...
<div>
  ...
  <list-card icon="menu" title="新闻资讯" :categories="newsList">
    <template #item="{category}">
      <router-link tag='div' :to="`/articles/${item._id}`" class="p-3 d-flex" v-for="(item, n) in category.newsList" :key="n">
    ...
    <template #item="{category}">
              <router-link tag='div' :to="`/articles/${item._id}`" class="d-flex flex-wrap" style="margin:0,-0.5rem" >
 class="d-flex flex-wrap" style="margin:0,-0.5rem" >
        <div style="width:18% " class="p-2 text-center" v-for="(item, n) in category.heroList" :key="n">
